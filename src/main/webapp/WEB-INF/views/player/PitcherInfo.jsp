 ...
 <!-- Header -->
  ...
  <div class="row g-0">
    <div class="col-md-2 p-3">
      <img src="${ppInfo.pimg }" class="img-fluid rounded-start" alt="..." style="width: 120px;">
    </div>
    <div class="col-md-10">
      <div class="card-body">
        <h5 class="card-title">
  ...
  <li class="list-group-item"> <strong>등번호: </strong> NO. ${ppInfo.pno }</li>
  <li class="list-group-item ">   <strong>포지션: </strong>${ppInfo.ppos }</li>
</ul>
     <ul class="list-group list-group-horizontal">
  <li class="list-group-item"> <strong>연봉: </strong> ${ppInfo.pmoney }</li>
...
<div class="scrapH">
		<input type="button" class="btn btn-lg scrap" id="${ppInfo.pcode }" onclick="checkVal('${ppInfo.pcode }', this)" value="♡ 관심선수 등록">
</div>
...
function checkVal(checkedName, selBtn){
	...
	if ($(selBtn).hasClass("scrap_click")) {
		...
		$.ajax({
			type: "get",
			url: "${pageContext.request.contextPath }/removepScrap",
	...
	$.ajax({
		type: "get",
		url: "${pageContext.request.contextPath }/scrapPname",
		data : {"checkedName" : checkedName},
		dataType : "json",
		success:function(result){
		}
	...
	function selectScrap(){
		$.ajax( { 
			type : "get",
			url : "${pageContext.request.contextPath }/selectpScrap",
			async:false,
			dataType:"json",
			success : function(scrapList){
					 for(var scrapinfo of scrapList){
					 $("#"+scrapinfo.pscpno).addClass("scrap_click");
					 }
